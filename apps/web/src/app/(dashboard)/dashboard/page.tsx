...
export default async function DashboardHome() {
  ...
  const [latestAudits, aiTraffic, quota] = await Promise.all([
  ...
  const scored = sites
    ...
    .filter(
      (row): row is { site: Site; audit: NonNullable<typeof row.audit> } => row.audit !== undefined,
    );
              ...
              {attention.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      style={{ fontWeight: 600, fontSize: 14, color: PC.ink, flex: '0 0 auto' }}
                    >
                      {r.site.name}
                    </span>
                    <span style={pill(tone.color, tone.bg)}>
                      {tone.label} {r.audit.score}
                    </span>
                    <span
                      style={{ fontSize: 14, color: PC.muted, flex: '1 1 auto', minWidth: 160 }}
                    >
                      {r.audit.failCount} check{r.audit.failCount === 1 ? '' : 's'} failing · agent
                      readiness {r.audit.agentReadinessScore}/8.
                    </span>
                    <span
                      style={{
                        display: 'inline-flex',
                        ...
                        fontSize: 13,
                        fontWeight: 500,
                        color: hasRepo ? PC.blazeDeep : PC.muted,
                        flex: '0 0 auto',
                      }}
                    >
                      {hasRepo ? (
        ...
        >
          {sites.map((s) => (
            <SiteCard
              key={s.id}
              site={s}
              audit={auditBySite.get(s.id)}
              spark={sparkBySite.get(s.id) ?? []}
            />
          ))}
        </div>
      </div>
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
        ...
        <span style={{ display: 'flex', flexDirection: 'column', minWidth: 0, lineHeight: 1.25 }}>
          ...
          <span style={ellipsis({ fontFamily: MONO, fontSize: 11, color: PC.dim })}>
      ...
      >
        ...
        <span style={{ color: verified ? PC.muted : PC.dim }}>
...
function RecentAuditsCard({
  ...
  readonly rows: ReadonlyArray<{
    site: Site;
    audit: { score: number; band: string; fetchedAt: Date };
  }>;
}) {
  return (
    <div
          ...
          return (
            ...
            >
              ...
              <span style={{ fontSize: 13, color: PC.ink, flex: '1 1 auto', minWidth: 0 }}>
                {r.site.name} scored{' '}
                <span style={{ fontFamily: MONO, color: tone.color }}>{r.audit.score}</span> (
                {tone.label})
              </span>
...
function Icon({
  ...
  size = 16,
  stroke = PC.dim2,
}: { readonly paths: ReactNode; readonly size?: number; readonly stroke?: string }) {
  return (
    <svg
      width={size}
      height={size}

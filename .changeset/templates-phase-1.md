---
"@answerable/templates": minor
---

Scaffold the `@answerable/templates` package — the fifth workspace package — and ship its first five page templates plus the token-substitution engine:
...
Each template is a complete Next.js App Router page that imports `defineSeo()` from `@answerable/metadata` and (where appropriate) a JSON-LD generator from `@answerable/schemas`. Drop the rendered file into `app/<name>/page.tsx` and you have a working, audit-ready page.
